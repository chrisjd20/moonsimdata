...
def layout_and_draw_abilities(draw, abilities, yaml_map, area_x, area_y, area_w, area_bottom):
    ...
    def measure(total_title_size, total_body_size):
        ...
        def token_width_sequence(val_text, color_code):
            # Mirror draw_token_sequence geometry: slightly taller-than-wide badges
            pad_x = 4
            tb = draw.textbbox((0, 0), val_text, font=body_font)
            text_w = tb[2] - tb[0]
            token_h_est = min(max(tb[3] - tb[1] + 12, line_h), line_h + 2)
            token_w = max(text_w + pad_x * 2, token_h_est - 2)
    ...
    for a in activated:
        ...
        draw.text((area_x, y), base_text, fill=(0, 0, 0, 255), font=title_font)
        nb = draw.textbbox((0, 0), base_text, font=title_font)
        # Draw tail in smaller italics on the same baseline
        if tail_text:
            tx = area_x + (nb[2] - nb[0])
            draw.text((tx, y), tail_text, fill=(0, 0, 0, 255), font=title_italic_font)
            nb = draw.textbbox((0, 0), base_text, font=title_font)
        y += (nb[3] - nb[1]) + 4
        desc = a.get('description', '') or ''
        for line in wrap_text(draw, desc, body_font, area_w):
    ...
    def draw_token_sequence(x, y, value_text, color_code, font):
        pad_x, pad_y, radius = 4, 6, 5
        white = (255, 255, 255, 255)
        line_h = draw.textbbox((0, 0), "Ag", font=font)[3]
        tb = draw.textbbox((0, 0), value_text, font=font)
        text_w = tb[2] - tb[0]
        text_h = tb[3] - tb[1]
        token_h = min(max(text_h + pad_y * 2, line_h), line_h + 2)
        token_w = max(text_w + pad_x * 2, token_h - 2)
    ...
    for a in arcane:
        ...
        if subtext:
            for line in wrap_text(draw, subtext, body_italic_font, area_w):
                draw.text((area_x, y), line, fill=(0, 0, 0, 255), font=body_italic_font)
...
def create_left_side_character_card(input_image_path, character_data, output_path):
    ...
    try:
        ...
        # Allow abilities content to approach very close to the pips row
