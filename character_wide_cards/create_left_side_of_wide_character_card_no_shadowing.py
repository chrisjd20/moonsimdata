...
def draw_stats_box(draw, x, y, width, character_data):
    ...
    for i, v in enumerate(values):
        ...
        draw.text((tx, base_y - 2), v_str, fill=(0, 0, 0, 255), font=value_font)
...
def draw_bottom_row(draw, text_left, bottom_boundary, text_right, character_data):
    ...
    draw.text((base_x + (base_w - (bb1[2] - bb1[0])), base_y_top), "Base:", fill=(0, 0, 0, 255), font=base_lbl_font)
    val_y = base_y_top + (bb1[3] - bb1[1]) + 2
    draw.text((base_x + (base_w - (bb2[2] - bb2[0])), val_y), bs_val, fill=(0, 0, 0, 255), font=base_val_font)
...
def layout_and_draw_abilities(draw, abilities, yaml_map, area_x, area_y, area_w, area_bottom):
    ...
    for a in passive:
        ...
        if lines:
            ...
            if label_w <= first_w:
                ...
                draw_text_with_special_symbols(draw, area_x, y, label, label_bold, (0, 0, 0, 255))
                draw_styled_text(draw, area_x + label_w, y, first[len(label):], body_font, label_bold, (0, 0, 0, 255))
            else:
                draw_styled_text(draw, area_x, y, first, body_font, label_bold, (0, 0, 0, 255))
        ...
        for line in lines[1:]:
            draw_styled_text(draw, area_x, y, line, body_font, label_bold, (0, 0, 0, 255))
            lb = draw.textbbox((0, 0), line, font=body_font)
            y += (lb[3] - lb[1]) + 2
            last_w = lb[2] - lb[0]
            last_h = lb[3] - lb[1]
        ...
        if tail:
            ...
            if tail_w <= remaining and last_h > 0:
                ...
                draw_text_with_special_symbols(draw, area_x + last_w, last_y_start, tail, body_italic_font, (0, 0, 0, 255), italic=True)
            else:
                # New line, left-aligned
                draw_text_with_special_symbols(draw, area_x, y, tail, body_italic_font, (0, 0, 0, 255), italic=True)
    ...
    for a in activated:
        ...
        draw_text_with_special_symbols(draw, area_x, y, base_text, title_font, (0, 0, 0, 255))
        nb = draw.textbbox((0, 0), base_text, font=title_font)
        # Draw tail in smaller italics on the same baseline
        if tail_text:
            tx = area_x + (nb[2] - nb[0])
            draw_text_with_special_symbols(draw, tx, y, tail_text, title_italic_font, (0, 0, 0, 255), italic=True)
            nb = draw.textbbox((0, 0), base_text, font=title_font)
        y += (nb[3] - nb[1]) + 4
        desc = a.get('description', '') or ''
        for line in wrap_text(draw, desc, body_font, area_w):
    ...
    def draw_token_sequence(x, y, value_text, color_code, font):
        ...
        if not cols:
            simple = f"{value_text}: "
            draw.text((x, y), simple, fill=(0, 0, 0, 255), font=token_bold_font)
            sb = draw.textbbox((0, 0), simple, font=token_bold_font)
            return x + (sb[2] - sb[0])
        for i, c in enumerate(cols):
            ...
            draw.text((tx, ty_text), value_text, fill=white, font=token_bold_font)
            x += token_w
            if i < len(cols) - 2:
    ...
    for a in arcane:
        ...
        if subtext:
            for line in wrap_text(draw, subtext, body_italic_font, area_w):
                draw_text_with_special_symbols(draw, area_x, y, line, body_italic_font, (0, 0, 0, 255), italic=True)
        ...
        for nc in non_cats:
            ...
            for w in words:
                ...
                if draw.textbbox((0, 0), test, font=body_font)[2] <= available_first or not first_line:
                    ...
                    break
            if first_line:
                draw_styled_text(draw, start_after_tokens_x, y, first_line, body_font, label_bold, (0, 0, 0, 255))
                lb = draw.textbbox((0, 0), first_line, font=body_font)
                y += (lb[3] - lb[1]) + 2
            remaining = desc_text[len(first_line):].lstrip()
            for cont in wrap_text(draw, remaining, body_font, area_w):
                draw_styled_text(draw, area_x, y, cont, body_font, label_bold, (0, 0, 0, 255))
                lb = draw.textbbox((0, 0), cont, font=body_font)
                y += (lb[3] - lb[1]) + 2
        if cata:
...
def create_left_side_character_card(input_image_path, character_data, output_path):
    ...
    try:
        ...
        if keywords and str(keywords).strip():
            ...
            draw.text((name_x, keywords_y), formatted, fill=black, font=keywords_font)
            ...
            if version_int is not None and version_int >= 1:
                ...
                draw.text((name_x, v_y), v_text, fill=black, font=version_font)
        ...
        final_rgba = Image.alpha_composite(img, text_overlay)
